...
import { LessonCategory, LessonItem, lessonKey } from 'src/app/services/lessons.service';
...
@Component({
  ...
  standalone: true,
  imports: [IonContent, IonHeader, IonTitle, IonToolbar, CommonModule, FormsModule, IonButtons, IonButton, IonIcon, IonSpinner, IonFooter]
})
export class VideoPlayerPage implements OnInit {
  ...
  ngOnInit() {
    if (this.item?.video) {
      ...
      const url = `${environment.apiBaseUrl}/video/${this.item.video}`;
      this.safeSrc = this.san.bypassSecurityTrustResourceUrl(url);
    }
  }
  ...
  onError(e: Event) {
    this.loading = false; // avoid spinner stuck
  ...
  onRateChange() {
    ...
    const rate = this.vid?.nativeElement?.playbackRate ?? 1;
    if (rate > 2 && !this.completionFired) {
      this.lastContinuousStart = this.vid.nativeElement.currentTime;
    }
  }
